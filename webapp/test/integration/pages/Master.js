		...
		onTheMasterPage: {
			...
			actions: {
				...
				iFilterTheListOnUnitNumber: function () {
					return this.iChooseASorter("filterButton", "<CategoryID>", "<100 ");
				},
				...
				iRememberAnIdOfAnObjectThatsNotInTheList: function () {
					return this.waitFor({
						...
						success: function (oList) {
							var aEntityData = this.getEntitySet("Categories");
							...
							if (aObjectsNotInTheList.length) {
								sObjectNotInTheListId = aObjectsNotInTheList[0].CategoryID;
							} else {
								// Not enough items all of them are displayed so we take the last one
								sObjectNotInTheListId = aEntityData[aEntityData.length - 1].CategoryID;
							}
							...
							oCurrentItem.bindingPath = "/" + oList.getModel().createKey("Categories", {
								CategoryID: sObjectNotInTheListId
							});
